...
func (s *webServiceImpl) configuration() {
	...
	for _, router := range routers {
		if router.TokenIsReq {
			s.Router.HandleFunc(router.Path, (util.Authenticate(router.EndPointer))).Methods(router.Method)
		}
		s.Router.HandleFunc(router.Path, (router.EndPointer)).Methods(router.Method)
	}
}
